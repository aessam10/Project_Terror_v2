...
namespace Project_Terror_v2.Game.MsgMonster
{
    ...
    public class MobItemGenerator
    {
        ...
        {
           uint amount = (uint)Program.GetRandom.Next(1, 200000);
           /* uint amount = 0;
            if (twin)
            {
                amount = (uint)Program.GetRandom.Next(300000, 800000);
            }
            else
            {
                if (normal)
                ...
                {
                    amount = (uint)Program.GetRandom.Next(0, (int)(((100000 * Family.Level / 2) / 2) + 1));
